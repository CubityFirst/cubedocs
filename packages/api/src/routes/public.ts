...
export async function handlePublic(
  ...
  if (parts[0] === "projects" && parts[1]) {
    ...
    const project = await env.DB.prepare(
      "SELECT id, name, description, published_at, vanity_slug, home_doc_id, graph_enabled, published_graph_enabled FROM projects WHERE (id = ? OR vanity_slug = ?) AND published_at IS NOT NULL",
  ...
  if (parts[0] === "docs" && parts[1] && parts[2]) {
    ...
    const project = await env.DB.prepare(
      "SELECT id, name, published_at, vanity_slug, home_doc_id, graph_enabled, published_graph_enabled FROM projects WHERE id = ? OR vanity_slug = ?",
    ).bind(projectIdOrSlug, projectIdOrSlug).first<Pick<PublicProject, "id" | "name" | "published_at" | "vanity_slug" | "home_doc_id" | "graph_enabled" | "published_graph_enabled">>();
    if (!project) return errorResponse(Errors.NOT_FOUND);
    ...
    return okResponse({
      ...
      sitePublished,
      project: { id: project.id, name: project.name, vanity_slug: project.vanity_slug ?? null, home_doc_id: project.home_doc_id ?? null, graph_enabled: project.graph_enabled, published_graph_enabled: project.published_graph_enabled },
      docs,
      folders,
      files,
    });
