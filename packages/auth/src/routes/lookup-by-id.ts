...
export async function handleLookupById(request: Request, env: Env): Promise<Response> {
  ...
  ).bind(body.userId).first<{ id: string; email: string; name: string; email_verified: number }>();

  if (!user) return errorResponse(Errors.NOT_FOUND);

  const emailVerificationEnabled = env.REQUIRE_EMAIL_VERIFICATION === "true";
  return okResponse({ userId: user.id, email: user.email, name: user.name, emailVerified: user.email_verified === 1, emailVerificationEnabled });
}
